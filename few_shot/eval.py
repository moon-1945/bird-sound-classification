...
def compute_prototypes(support: torch.Tensor, encoder) -> torch.Tensor:
    """
    support: [N, K, 1, T]
    returns: [N, D] — one prototype per class, mean of K support embeddings
    """
    N, K, C, T = support.shape
    # flatten N and K into one batch for a single encoder forward pass
    flat = support.reshape(N * K, C, T)               # [N*K, 1, T]
    embeddings = encoder.embed(flat)                   # [N*K, D]
...
def classify_queries(
    ...
    embeddings = encoder.embed(flat)                   # [N*Q, D]
...
def episode_accuracy(
    ...
    prototypes = compute_prototypes(support, encoder)
    preds = classify_queries(query, prototypes, encoder)
...
def evaluate_encoder(
    ...
    for support, query, labels in tqdm(sampler):
        ...
        acc = episode_accuracy(support, query, labels, encoder)
        accs.append(acc)
...
def main():
    ...
    parser.add_argument("--k_shot", type=int, nargs="+", default=[1, 5, 10],
    ...
    with torch.no_grad():
        for cfg in MODEL_CONFIGS:
            ...
            for k_shot in args.k_shot:
                ...
                result = evaluate_encoder(encoder, sampler, device)
